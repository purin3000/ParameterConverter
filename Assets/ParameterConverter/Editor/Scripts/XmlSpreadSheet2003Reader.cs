...
using System.IO;
using System.Xml;
using ParameterConverter.Editor.Utils;

namespace ParameterConverter.Editor {
  /// <summary>
  /// エクセルからXMLスレッドシートで出力したXMLからCellDataを生成
  /// </summary>
  public static class XmlSpreadSheet2003Reader {
    public static List<CellData> CreateCellData(string assetPath) {
      ...
      using (var stream = new FileStream(assetPath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
        ...
        if (worksheets == null) {
          throw new Exception($"ワークシートが存在しません {assetPath}");
        }
    ...
    private static string[,] ParseTable(XmlElement table, XmlNamespaceManager nsmgr) {
      ...
      foreach (var (rowIndex, rowNode) in rowNodes.Indexed<XmlElement>()) {
        var cellNodes = rowNode.SelectNodes("a:Cell", nsmgr);
        var colOffset = 0;
        foreach (var (columnIndex, cellElem) in cellNodes.Indexed<XmlElement>()) {
