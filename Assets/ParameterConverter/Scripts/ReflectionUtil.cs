...
namespace ParameterConverter.Scripts {
  ...
  public interface ILexicalCastHook {
    bool Cast(Type scriptableObjectType, FieldInfo fieldInfo, Type targetType, object val, int arrayIndex,
      out object obj, IErrorInfo errorInfo);
  }

  internal static class ReflectionUtil {
    private static readonly Regex RegSplit = new(@"^([^.]+)\.(.+)");
    private static readonly Regex RegArray = new(@"^([^.\[\]]+)\[(\d+)\]");
    ...
    public static void SetFieldValue(Type t, object obj, string name, object value,
      IEnumerable<ILexicalCastHook> lexicalCastHooks, IErrorInfo errorInfo) {
      ...
      if (m.Success) {
        ...
        if (arrayIndex == -1) {
          ...
          if (fieldInfo.FieldType.IsValueType) // 値型の場合はBoxsingの都合でコピーに対して設定されているので、コピーを設定しなおす必要がある
            fieldInfo.SetValue(obj, fieldObj);
        } else {
          // 配列である
          ...
          if (elementType.IsValueType) // 値型の場合はBoxsingの都合でコピーに対して設定されているので、コピーを設定しなおす必要がある
            arrayInstance.SetValue(elementObj, arrayIndex);
        }
      } else {
        // 最後の要素である
        ...
        if (arrayIndex == -1) {
          ...
          // フィールドに値を設定
          var newValue = LexicalCast2(t, fieldInfo, fieldInfo.FieldType, value, -1, lexicalCastHooks,
            errorInfo);
          ...
          // 配列に値を設定
          var newValue = LexicalCast2(t, fieldInfo, fieldInfo.FieldType.GetElementType(), value, arrayIndex,
            lexicalCastHooks, errorInfo);
          arrayInstance.SetValue(newValue, arrayIndex);
        }
      }
    }

    public static object LexicalCast2(Type scriptableObjectType, FieldInfo fieldInfo, Type targetType, object value,
      int arrayIndex, IEnumerable<ILexicalCastHook> lexicalCastHooks, IErrorInfo errorInfo) {
      foreach (var hook in lexicalCastHooks)
        if (hook.Cast(scriptableObjectType, fieldInfo, targetType, value, arrayIndex, out var newValue,
              errorInfo))
          return newValue;
    ...
    public static object LexicalCast(Type targetType, object value, IErrorInfo errorInfo) {
      if (targetType.IsEnum) {
        ...
        if (!string.IsNullOrEmpty(label)) {
          ...
          if (index != -1) return Enum.GetValues(targetType).GetValue(index);
    ...
    /// <returns></returns>
    private static (FieldInfo, string fieldName, int arrayIndex) GetFieldInfo(Type t, string name,
      ...
      if (fieldInfo == null) throw new FieldNameException(fieldName, errorInfo);
